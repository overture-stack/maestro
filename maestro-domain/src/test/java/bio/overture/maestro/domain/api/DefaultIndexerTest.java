...
class DefaultIndexerTest {
  ...
  @Mock private ExclusionRulesDAO exclusionRulesDAO;

  @Mock private StudyDAO studyDAO;

  @Mock private FileCentricIndexAdapter indexServerAdapter;
  ...
  void indexStudyshouldNotifyOnStudyFetchError() {
    ...
    // Then
    StepVerifier.create(indexResult).expectNext(output).expectComplete().verify();
  ...
  void indexRepositoryShouldHandleFetchStudiesFailure() {
    ...
    given(studyDAO.getStudies(eq(getStudiesCmd)))
        .willReturn(Flux.error(new RuntimeException("sike!")));
    ...
    // Then
    StepVerifier.create(indexResultMono).expectNext(failedIndexResult).expectComplete().verify();
  ...
  void shouldExcludeSampleIdFromIndexing() {
    ...
    val fileCentricDocuments =
        Arrays.asList(
            loadJsonFixtureSnakeCase(
                getClass(),
                studyId + ".files.excluded.SA520221.json",
    ...
    val batchIndexFilesCommand =
        BatchIndexFilesCommand.builder().files(fileCentricDocuments).build();
    val getStudyAnalysesCommand =
        GetStudyAnalysesCommand.builder()
            .studyId(studyId)
            .filesRepositoryBaseUrl(filesRepository.getUrl())
            .build();

    Mono<Map<Class<?>, List<? extends ExclusionRule>>> sampleExclusionRule =
        Mono.just(
            Map.of(Sample.class, List.of(new IDExclusionRule(Sample.class, List.of("SA520221")))));
    ...
    // Then
    StepVerifier.create(indexResultFlux).expectNext(result).expectComplete().verify();
  ...
  void shouldDetectConflictInFileAndDeleteItFromIndex() {
    ...
    val repoCode = "TEST-REPO";
    val filesRepository = getStubFilesRepository();
    // Note: in MALE-DE.conflicting.analysis.json, conflict is: analysis
    // 0a1df2a2-029d-48cc-839c-0a7c89ff972f is UNPUBLISHED
    val a1 =
        Arrays.asList(
            loadJsonFixture(getClass(), studyId + ".conflicting.analysis.json", Analysis[].class));
    ...
    val expectedNotification =
        new IndexerNotification(
            NotificationName.INDEX_FILE_CONFLICT, getConflicts(fileCentricDocuments));
            ...
            DefaultIndexer.FileConflict.builder()
                .indexedFile(
                    DefaultIndexer.ConflictingFile.builder()
                        ...
                        .repoCode(
                            fileCentricDocuments.get(0).getRepositories().stream()
  ...
  @Test
  void shouldIndexSingleAnalysisWithCRAMFiles() {
    // Given
    val studyId = "CRAM-TEST";
    val repoCode = "TEST-REPO";
    val analysisId = "EGAZ00001254369";
    val filesRepository = getStubFilesRepository();
    val a1 =
        loadJsonFixture(getClass(), studyId + ".analysis.EGAZ00001254369.json", Analysis.class);
    val fileCentricDocuments = getExpectedFileCentricDocument(studyId);
    val fileRepo = Mono.just(getStubFilesRepository());
    val studyAnalysis = Mono.just(a1);
    val result = IndexResult.builder().successful(true).build();
  ...
  void shouldNotIndexAnalysisIfExcludedByRule() {
    ...
    val getStudyAnalysesCommand =
        GetAnalysisCommand.builder()
            ...
            .filesRepositoryBaseUrl(filesRepository.getUrl())
            .build();
    val sampleExclusionRule =
        Mono.<Map<Class<?>, List<? extends ExclusionRule>>>just(
            Map.of(
                Analysis.class,
                List.of(new IDExclusionRule(Analysis.class, List.of("EGAZ00001254368")))));
  ...
  void indexRepositoryshouldNotifyOnStudyFetchError() {
    ...
    given(studyDAO.getStudies(eq(getStudiesCmd)))
        .willReturn(Flux.error(new IndexerException("failed", new RuntimeException(""), failure)));
    given(studyRepositoryDao.getFilesRepository(eq(repoCode))).willReturn(fileRepo);
  ...
  private List<FileCentricDocument> getExpectedFileCentricDocument(String studyId) {
    return Arrays.asList(
        loadJsonFixtureSnakeCase(getClass(), studyId + ".files.json", FileCentricDocument[].class));
  }
