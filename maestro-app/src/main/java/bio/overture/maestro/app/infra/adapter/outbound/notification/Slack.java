...
public class Slack implements NotificationChannel {
  ...
  public Mono<Boolean> send(@NonNull IndexerNotification notification) {
    ...
    switch (notification.getNotificationName().getCategory()) {
      case ERROR:
        template = this.slackChannelInfo.errorTemplate();
        break;
      case WARN:
        template = this.slackChannelInfo.warningTemplate();
        break;
    }
    ...
    val dataStringTruncated =
        dataString.substring(
            0, Math.min(dataString.length(), this.slackChannelInfo.maxDataLength()));
