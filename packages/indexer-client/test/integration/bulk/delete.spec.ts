...
export default function suite() {
	...
	it('should return successful true when bulk delete data', async () => {
		...
		// Delete Data
		const idsToDelete: string[] = ['1', '2', '3'];
		const bulkRequestDelete: DeleteBulkRequest[] = idsToDelete.map((id) => ({
			action: BulkAction.DELETE,
			id,
		}));

		const resultDelete = await client.bulk(indexName, bulkRequestDelete);
		expect(resultDelete.successful).to.eql(true);
		expect(resultDelete.indexName).to.eql(indexName);
		expect(Object.keys(resultDelete.failureData).length).to.eq(0);
	});

	it('should return successful true when doc records to delete does not exists', async () => {
		const indexName = 'test-index';

		// Delete Data
		const doesNotExistIds: string[] = ['1', '2', '3'];
		const bulkRequest: DeleteBulkRequest[] = doesNotExistIds.map((id) => ({
			action: BulkAction.DELETE,
			id,
		}));
	...
	it('should return successful false when a ConnectionError is thrown', async () => {
		...
		const idsToDelete: string[] = ['1', '2', '3'];
		const bulkRequest: DeleteBulkRequest[] = idsToDelete.map((id) => ({
			action: BulkAction.DELETE,
			id,
		}));
