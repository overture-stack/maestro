...
class Notifier {
    ...
    public void notify(IndexerNotification notification) {
        ...
        // the caller (indexer) doesn't need to worry about what happens here so no need to return the flux and
        // we subscribe here.
        Flux.fromIterable(getEligibleChannels(notification))
            .flatMap(notificationChannel -> sendNotificationThroughChannel(notification, notificationChannel))
            .subscribe();
    }

    private Mono<Boolean> sendNotificationThroughChannel(IndexerNotification notification, NotificationChannel notificationChannel) {
        return notificationChannel.send(notification)
            .onErrorResume(e -> {
    ...
    private boolean shouldReceiveNotification(IndexerNotification notification, NotificationChannel notificationChannel) {
