...
final class AnalysisCentricDocumentConverter {
  ...
  public static List<AnalysisCentricDonor> getDonors(@NonNull Analysis analysis) {
    ...
    return groupedByDonorMap.values().stream()
   ...
   * @return a fully assembled AnalysisCentticDonor object with a list of specimens that belongs to
   *     the current donor
   */
  private static AnalysisCentricDonor mergeDonorBySpecimen(
      @NonNull List<AnalysisCentricDonor> list) {

    checkNotFound(
        list.size() > 0, "Failed to merge AnalysisCentricDonor by specimen: donor list is empty.");
        ...
        specimenList.stream()
            .collect(
                Collectors.groupingBy(
                    bio.overture.maestro.domain.entities.indexing.Specimen::getSpecimenId,
                    Collectors.toList()));

    val specimens =
        specimenMap.values().stream()
            .collect(Collectors.toList())
            .stream()
  ...
  public static bio.overture.maestro.domain.entities.indexing.Specimen groupSpecimensBySample(
   ...
   * @return
   */
  public static List<bio.overture.maestro.domain.entities.indexing.Specimen> buildSpecimen(
      @NonNull Specimen specimen, @NonNull Sample sample) {
    return List.of(
