...
/**
 * Rules dao backed by application properties as source, it assumes a yaml structure matching the
 * class: {@link RuleConfig}
 */
@Slf4j
public class ApplicationPropertiesExclusionRulesDAO implements ExclusionRulesDAO {
  ...
  private void init(Map<String, List<String>> idExclusionRules) {
    try {
      ...
      idExclusionRules.forEach(
          (entity, ids) -> {
            if (ids.isEmpty()) return;
            val entityClass = getClassFor(entity);
            rulesByEntity.put(
                entityClass,
                List.of(IDExclusionRule.builder().clazz(entityClass).ids(ids).build()));
          });
  ...
  private Class<?> getClassFor(String entity) {
    ...
    throw new IllegalArgumentException(
        format("entity : {0} is not recognized for exclusion rules", entity));
  }
