...
/**
 * Properties files backed repository store, reads the information by binding to the
 * application.config files property: maestro.repositories the configurable attributes can be found
 * here: {@link PropertiesFileRepository}
 *
 * <p>this serves as a default in memory store, more sophisticated cases may create a DB backed
 * store.
 */
@Slf4j
@Getter
...
class PropertyFileStudyRepositoryDAO implements StudyRepositoryDAO {
  ...
  public Mono<StudyRepository> getFilesRepository(@NonNull String code) {
    val repository =
        repositories.stream()
            .filter(
                propertiesFileRepository ->
                    propertiesFileRepository.getCode().equalsIgnoreCase(code))
            .distinct()
