...
export default function suite() {
	...
	it('should return successful true when bulk creating data', async () => {
		...
		// Update Data
		const recordsUpdate: DataRecordNested[] = [
			{ id: 1, name: 'VALUE1' },
			{ id: 2, name: 'VALUE2' },
			{ id: 3, name: 'VALUE3' },
		];
		const bulkRequestUpdate: UpdateBulkRequest[] = recordsUpdate.map((data) => ({
			action: BulkAction.UPDATE,
			dataSet: data,
		}));

		const resultUpdate = await client.bulk(indexName, bulkRequestUpdate);
		expect(resultUpdate.successful).to.eql(true);
		expect(resultUpdate.indexName).to.eql(indexName);
		expect(Object.keys(resultUpdate.failureData).length).to.eq(0);
	});
	...
	it('should return successful false when doc id does not exists', async () => {
		...
		const bulkRequest: UpdateBulkRequest[] = records.map((data) => ({
			action: BulkAction.UPDATE,
			dataSet: data,
		}));
		...
		expect(result.failureData).to.eql({ error: ['ResponseError'] });
	});
	...
	it('should return successful false when a ConnectionError is thrown', async () => {
		...
		const records: DataRecordNested[] = [{ name: 'value1' }, { name: 'value2' }, { name: 'value3' }];
		const bulkRequest: UpdateBulkRequest[] = records.map((data) => ({ action: BulkAction.UPDATE, dataSet: data }));
