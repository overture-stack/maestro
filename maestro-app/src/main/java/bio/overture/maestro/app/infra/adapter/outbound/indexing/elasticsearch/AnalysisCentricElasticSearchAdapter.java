...
public class AnalysisCentricElasticSearchAdapter implements AnalysisCentricIndexAdapter {
  ...
  @Inject
  public AnalysisCentricElasticSearchAdapter(RestHighLevelClient elasticsearchRestClient,
  ...
  @Override
  public Mono<IndexResult> batchUpsertAnalysisRepositories(@NonNull BatchIndexAnalysisCommand batchIndexAnalysisCommand) {
    log.debug("in batchUpsertAnalysisRepositories, analyses count: {} ", batchIndexAnalysisCommand.getAnalyses().size());
    return  Mono.fromSupplier(() -> this.bulkUpsertAnalysisRepositories(batchIndexAnalysisCommand.getAnalyses()))
  ...
  private IndexResult bulkUpsertAnalysisRepositories(List<AnalysisCentricDocument> analyses) {
    ...
    val failures = Parallel
            .blockingScatterGather(analyses, this.documentsPerBulkRequest, this :: tryBulkUpsertRequestForPart)
            .stream()
            .flatMap(Set::stream)
  ...
  @NotNull
  private Set<String> tryBulkUpsertRequestForPart(Map.Entry<Integer, List<AnalysisCentricDocument>> entry) {
  ...
  @SneakyThrows
  private UpdateRequest mapAnalysisToUpsertRepositoryQuery(AnalysisCentricDocument analysisCentricDocument){
    ...
    return new UpdateRequest()
            ...
            .upsert(new IndexRequest()
                    .index(this.alias)
                    .id(analysisCentricDocument.getAnalysisId())
                    .source(analysisCentricJSONWriter.writeValueAsString(analysisCentricDocument), XContentType.JSON)
            );
  }
